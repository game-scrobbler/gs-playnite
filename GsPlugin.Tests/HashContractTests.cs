...
namespace GsPlugin.Tests {
    ...
    public class HashContractTests {
        private static JsonElement LoadFixture() {
            // Assembly.CodeBase is .NET Framework only and obsolete on .NET 10; Location is the
            // supported way to find the test binary's own directory.
            var path = Path.Combine(
                Path.GetDirectoryName(typeof(HashContractTests).Assembly.Location)!,
                "Fixtures",
