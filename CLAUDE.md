...
- Telemetry starts after saved preferences are loaded. Settings save and deletion reconcile SDK lifetimes; opt-back-in does not, because `HasConsent` stays false while `PendingRestartAfterOptIn` is set. Consent handlers check every outgoing batch, including automatic sessions; a retired SDK lifetime stays revoked even after preferences are re-enabled. `PerformOptIn` returns false and restores `OptedOut` if the durable save fails.
