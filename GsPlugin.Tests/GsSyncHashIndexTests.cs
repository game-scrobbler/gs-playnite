...
namespace GsPlugin.Tests {
    ...
    public class GsV4ChunkedSyncClientTests {
        private sealed class TrackingMockApiClient : IGsApiClient {
            ...
            public Task<GameDataRes> GetGameData(string playniteGameId) => Task.FromResult<GameDataRes>(null);
        ...
        private static GsScrobblingService CreateService(IGsApiClient client) {
            // These tests drive prebuilt DTOs through the upload paths, which never touch
            // the library collections, so no ILibraryApi is needed.
            return new GsScrobblingService(client, null, null);
        }
